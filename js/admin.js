...
function addThongKe() {
    ...
    danhSachDonHang.forEach(donHang => {
        ...
        donHang.sp.forEach(sanPhamTrongDonHang => {
            if (!sanPhamTrongDonHang.sanPham) return; // Nếu sản phẩm không tồn tại thì bỏ qua
    ...
    for (var i = 0; i < u.length; i++) {
        for (var j = 0; j < u[i].donhang.length; j++) {
            ...
            for (var s of u[i].donhang[j].sp) {
                ...
                if (!timsp) continue; // Nếu sản phẩm không tồn tại thì bỏ qua
                if (timsp.promo && timsp.promo.name == 'giareonline')
            ...
            // for (var s of u[i].donhang[j].sp) {
            //     var timsp = timKiemTheoMa(list_products, s.ma);
            //     if (!timsp) continue; // Bỏ qua nếu sản phẩm không còn
            //     if (timsp.promo && timsp.promo.name == 'giamgia')
            //         tongtien += stringToNum(timsp.promo.value);
            //     else
            //         tongtien += stringToNum(timsp.price);
            // }
            ...
            var sps = '';
            for (var s of u[i].donhang[j].sp) {
                var spObj = timKiemTheoMa(list_products, s.ma);
                if (!spObj) continue; // Nếu sản phẩm không tồn tại thì bỏ qua
                sps += `<p style="text-align: right">` + (spObj.name + ' [' + s.soluong + ']') + `</p>`;
            }
